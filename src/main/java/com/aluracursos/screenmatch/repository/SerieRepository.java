...
//  List<Serie> findByTotalDeTemporadasLessThanEqualAndEvaluacionGreaterThanEqual(int totalDeTemporadas, Double evaluacion);
    ...
    @Query("SELECT e FROM Serie s JOIN s.episodios e WHERE LOWER(e.titulo) = LOWER(:nombreEpisodio)")
    List<Episodio> episodiosPorNombreExacto(@Param("nombreEpisodio") String nombreEpisodio);
